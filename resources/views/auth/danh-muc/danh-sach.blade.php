        ...
        <div class="container-xl">
            <div class="row g-2 align-items-center">
                ...
                <div class="col-auto ms-auto d-print-none">
                    <a href="{{action('App\Http\Controllers\DanhMucController@getViewThem')}}" class="btn btn-primary">
                ...
                <div class="col-lg-12">
                    <div class="card">
                                ...
                                <table class="table table-striped">
                                    ...
                                    @foreach($dm as $item)
                                        <tr id="{{$item->id_danh_muc}}">
                                            ...
                                            <td class="w-0">
                                                <span class="text-nowrap">
                                                    <a href="{{action('App\Http\Controllers\DanhMucController@getViewCapNhat',['id_danh_muc'=>$item->id_danh_muc])}}" class="text-decoration-none itemCapNhat">
                                                        @component('auth.icons.edit')@endcomponent
                                                    </a>
                                                    <span class="link-primary">|</span>
                                                    <a href="#" data="{{$item->id_danh_muc}}" class="itemXoa">
                        ...
                        <div class="card-footer">
                            {{$dm->links('pagination::bootstrap-5')}}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('.itemXoa').click(function () {
                if (!confirm("Danh mục bị xóa đồng thời các dữ liệu liên quan đến danh mục này sẽ bị xóa theo\nSau khi xóa dữ liệu sẽ không thể phục hồi lại được.\nXác nhận xóa?\n")) {
                    return;
                }
                var id = $(this).attr('data');
                $.ajax({
                    url: '{{action('App\Http\Controllers\DanhMucController@deleteDanhMuc')}}',
                    type: "DELETE",
                    data: {
                        'id_danh_muc': id,
                    },
